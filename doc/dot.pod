...
Set the symbolic links interactively. This command sets symbolic links configured in F<~/.config/dot/dotlink>.
...
Remove the all symbolic links in F<~/.config/dot/dotlink>.

=item B<config>

Edit (or create if the file doesn't exist) rcfile F<~/.config/dot/dotrc>.
...
Specify configuration file to overload.
